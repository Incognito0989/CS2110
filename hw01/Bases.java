...
public class Bases
{
    ...
    public static int decimalStringToInt(String decimal)
    {
        int num = 0;
        int n = 0;
        while(n < decimal.length()) {
            num *= 10;                          //'4' == 110 100
            num += (decimal.charAt(n++) ^ '0'); //'0' == 110 000
        }
        return num;
    }
    ...
    {
        int num = 0;
        for(int n = 0; n < octal.length(); n++) { //for each octal digit
            int p = octal.length() - n - 1;
            num += (octal.charAt(n) ^ '0') << p << p << p;  //1111
        }
        return num;
    }
