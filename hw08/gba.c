...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for(int n = 0; n < height; n++) {
    DMA[3].dst = videoBuffer + OFFSET(row + n, col, WIDTH);
    //DMA[3].dst = &videoBuffer[OFFSET(row + n, col, WIDTH)];
    DMA[3].src = &image[OFFSET(n, 0, width)];
...
//say we have a full screen background. move something on top of it gotta erase and remake. Dont draw screen all over again. redraw a spall cropped area instead.
