...
EditImageInfo::EditImageInfo(const std::string& _filename) : filename(_filename)
{
    std::string name = Format(filename);
    std::vector<Magick::Image> imagesMagick;
    readImages(&imagesMagick, filename);
    for (auto image : imagesMagick)
        images.emplace_back(Image32Bpp(image, name, filename, image.scene(), images.size() > 1));
}
