...
int set_character(struct ascii_image *image, int row, int col, char c)
{
    ...
    //UNUSED_PARAM(col);
    //UNUSED_PARAM(c);
    if(image == NULL || image->data == NULL || is_printable(c) == 0 || row < 0 || row > image->height - 1 || col < 0 || col > image->width - 1) {
        return FAILURE;
    }
    int offset = ((col) + (image->width)*(row));
    image->data[offset] = c;
    return SUCCESS;
...
int draw_diagonal_line(struct ascii_image *image, int row1, int col1, int row2, int col2, char c)
{
    ...
    //UNUSED_PARAM(col2);
    //UNUSED_PARAM(c);
    if(image == NULL || image->data == NULL || is_printable(c) == 0) {
        return FAILURE;
    }
    if(row1 < 0 || row2 < 0 || row1 > image->height - 1 || row2 > image->height - 1 || col1 < 0 || col1 > image->width - 1 || col2 < 0 || col2 > image->width - 1) {
        return FAILURE;
    }
    ...
    while(row1 != row2 && col1 != col2) {
        set_character(image, row, col, c);
        row += (row2 - row1);
        col += (col2 - col1);
        offset = ((col) + (image->width)*(row));
    }
    /**Implement checks for failure cases as detailed in the method description**/
...
int replace_extension(struct ascii_image *image, char *extension)
{
    ...
    if(strlen(image->name) == 0) {
        struct ascii_image *p = create_image(image->height, image->width, extension);
        if(p == NULL) {
            return FAILURE;
        }
        image->name = p->name;
    ...
    int i = 1;
    for(int n = strlen(image->name) - 1; n >= 0; n--) {
        if(image->name[n] == '.') {
            char n[strlen(image->name) - i];
            strncpy(image->name, n, strlen(image->name) - i - 1);
            strcat(n, extension);
            struct ascii_image *p = create_image(image->height, image->width, n);
            if(p == NULL) {
                return FAILURE;
            }
            destroy_image(image);
